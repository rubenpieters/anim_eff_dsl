...
\subsubsection{LinearTo}

\paragraph{Examples}

To illustrate the behaviour of running animations, we show how the implementation operates in a very simplified setting. The application state is a tuple, \hs{type State = (Float, Float)}, where we call the first component \hs{x} and the second component \hs{y}. We define a simple animation \hs{moveRight} which transforms the \hs{x}-value to 50 over a time of 1 second.
