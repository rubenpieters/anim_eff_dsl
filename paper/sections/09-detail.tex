...
cases, we leave the freedom to opt for an alternative like \hs{Applicative f} in
other cases.
...
The \hs{liftA2} function from the \hs{Applicative} class, which has type \\\hs{Applicative f => (a -> b -> c) -> f a -> f b -> f c}, takes two animations \hs{f a} and \hs{f b} and returns a new animation which plays them in order. The final result of the animation is of type \hs{c}, which is obtained by using the function \hs{a -> b -> c} on the results of the two played animations.

The \hs{>>=} function from the \hs{Monad} class (pronounced 'bind'), which has type \hs{Monad f => f a -> (a -> f b)}, takes an animation \hs{f a} and then feeds the result of this animation into the function \hs{a -> f b} to play the animation \hs{f b}.


The \hs{sequential} function used earlier is a specialization of the \hs{liftA2} function. It does not require a \hs{a -> b -> c} function, and is only applicable to animations return a trivial value of type \hs{()}.
