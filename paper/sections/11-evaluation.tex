...
\hs{parallel} composition, we can define in terms of other GSAP features.
...
we can define similar functionality as \hs{sequential} and \hs{parallel} in
\dsl{}.
...
However, if we want to provide animations similar to \hs{onlyDone}, which
contains an \hs{if-then-else}, then the duration returned is not what we
